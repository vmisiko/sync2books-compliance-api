...
**Update 3 (2026-08-12):** started a fifth session, new Application Test Pin `P600004151A`, same device serial `JM9QLXNJ75`. First `/initialize` attempt of this fresh session failed with the identical error (`"2 results were returned"`). This is the fifth distinct pin to hit this exact bug against this device serial, over multiple days — confirming the underlying corruption has not been cleaned up on KRA's side between sessions. We made exactly one `/initialize` attempt this time and stopped immediately rather than retry.

**Update 4 (2026-08-12, corrected) — resolved on our end, closing this ticket.** An earlier version of this update reported testing against a brand-new Apigee app (App ID `0ada03d4-8b15-4c51-9a6b-7bdbe8ce2e78`, new Application Test Pin `P600004152A`) and getting the same error. That test was invalid — a stale server process on our side was still running against the *previous* Apigee app's credentials, so the request never actually reached KRA. After fixing our tooling and re-testing properly against the genuinely new app, `/initialize` succeeded (HTTP 200) for `dvcSrlNo` `JM9QLXNJ75` under pin `P600004152A` — first attempt, no error. This means the corruption was scoped to session state accumulated under the **previous Apigee app** (`e0c07d61-386c-4628-8378-3a2874f14cc0`, and before that `cff33b56-c663-48b9-80d1-d7d1ffae019c`), not to the device serial itself as we'd previously concluded — registering a new Apigee app was in fact the fix, we just hadn't verified it correctly the first time we tried it. Thank you for your patience; no further action needed on your side for this specific issue.
