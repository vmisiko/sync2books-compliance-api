...
**The developer-facing API key surface does not exist yet.** The plan is `.docs/COMPLIANCE_SELF_SERVE_API_PLATFORM_PLAN.md` (P1: `compliance_applications`, hashed `compliance_api_keys`, a tenant-scope guard and a versioned `/v1` surface). Until it ships, "a merchant's developer gets an API key from the compliance dashboard" is a target, not a description — don't write docs or code comments that state it in the present tense.
