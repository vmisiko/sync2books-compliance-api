...
**Developer API keys (P1, built 2026-09-21; `src/developer-platform/`).** A merchant organisation creates an *application* and issues *API keys* (`cmp_sk_test_…` / `cmp_sk_live_…`, stored only as a SHA-256 hash, shown once) through `dashboard-api/developer/*` (session-authenticated). Public routes live under `/v1/*` and are mounted behind three guards in this order: `ComplianceApiKeyGuard` (authenticate + scopes) → `ApiRateLimitGuard` (per-application, real `X-RateLimit-*` headers, Redis when `COMPLIANCE_RATE_LIMIT_REDIS_URL`/`ETIMS_OSCU_REDIS_URL` is set, per-process otherwise, logged at warn) → `TenantScopeGuard` (business must belong to the key's organisation **and** its eTIMS environment must match the key's). Every `/v1` route must carry `@RequireScopes(...)`. Only `GET /v1/me` and `GET /v1/businesses[/:id]` exist so far; the rest of the surface (P2), the dashboard Developers UI (P3) and webhooks (P4) are not built — so the *self-service* flow is still not present-tense for a human in a browser, only over the session API. `Idempotency-Key` parsing exists (`@IdempotencyKey()`) but is not yet wired to a route. Plan: `.docs/COMPLIANCE_SELF_SERVE_API_PLATFORM_PLAN.md`.
